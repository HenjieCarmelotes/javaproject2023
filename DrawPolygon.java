...
public class DrawPolygon extends Application {
	 ...
	 public static void main(String[] args) {
	    ...
	    private void draw() {
	        ...
	        g.fillRect(0,0,canvas.getWidth(),canvas.getHeight()); //fill the drawing area with white.
	        if (pointCt == 0)
	            return;
	        g.setLineWidth(2);
	        ...
	        if (complete) { // draw a polygon
	            ...
	            for (int i = 0; i < pointCt - 1; i++) {
	                g.strokeLine( xCoord[i], yCoord[i], xCoord[i+1], yCoord[i+1]); //the 0ith is the first point that contains the mouse event x & y values.
	            }
	        }
	    }
	    ...
	    private void mousePressed(MouseEvent evt) { 
	        ...
	        else if ( pointCt > 0 && (Math.abs(xCoord[0] - evt.getX()) <= 3)
